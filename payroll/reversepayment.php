...
if(isset($_POST['submit'])){
 ...
 $sql = "SELECT * FROM payment WHERE month='$months' AND year = '$year' ";
 $result = mysqli_query($conn,$sql);

 if(mysqli_fetch_assoc($result) < 1){

    echo "<script> alert('There are no recorded payment for this month') </script>";
} else {
   $sql=" DELETE FROM `payment` WHERE month='$months'"; 
   $result = mysqli_query($conn,$sql);
   if($result){
    echo "<script> alert('The records were removed successfully') </script>";
   }
  }
}
?>
...
<body>
  <div class="jumbotron text-center">
    <h1> payroll reverse payment page </h1>
