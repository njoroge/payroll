...
if(isset($_POST['submit'])){
    $dptid = $_POST['dpid'];
    $dptname =$_POST['dpname'];

    $sql= "SELECT *FROM departments WHERE depart_id = '$dptid' AND depart_name = '$dptname'";

    $result = mysqli_query($conn, $sql);

    if(mysqli_fetch_assoc( $result)>1){
        echo  "<script> alert( ' THE BANK ALREADY EXISTS...') </script>";
     }else{ 

        $sql = "INSERT INTO departments(depart_id, depart_name, depart_status) VALUES ('$dptid','$dptname','ACTIVE')";

        $result = mysqli_query($conn, $sql);

        if ($result){
            echo  "<script> alert( ' THE DEPARTMENT REGISTERED SUCCESSFULLY') </script>";
     }else{ 
    ...
    <form action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>" method="post">
      ...
      <div class="form-row">
        <div class="col">
          ...
          <label for="bank id"><b>Department Id:</b></label>
          <input type="text" placeholder="Enter the department id" name="dpid" required>
        </div>
      </div>
      ...
      <div class="form-row">
        <div class="col">
          ...
          <label for="bank name"><b>Department Name:</b></label>
          <input type="text" placeholder="Enter the department name" name="dpname" required>
        </div>
      </div>
