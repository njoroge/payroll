...
$sql = "SELECT * FROM empdetail WHERE nationalid ='$natid' AND work_status = 'ACTIVE' ";
...
if (mysqli_fetch_assoc($result) < 1) {
    ...
    if ($admt > 20000) {
        echo "<script> alert('The amount is too high') </script>";
    } else {
        $sql1 = " INSERT INTO advance (nationalid,advance_amount,date_paid) VALUES ('$natid','$admt',now() )";

        $result = mysqli_query($conn, $sql1);

        if ($result) {
            echo "<script> alert( 'Allowance allocated successfully...') </script>";
        } else {
...
<body>
  <div class="jumbotron text-center">
    <h1> payroll advance payment page </h1>
...
<label for="id number "><b>National ID:</b></label>
      <input type="number" placeholder="enter the national id number." name="natid" required>
    <div>
      <label for="advance amount"><b>advance amount:</b></label>
          <input type="number" placeholder="Enter the advance amount " name="adamount" required>
</div>
</div>
