...
if(isset($_POST['submit'])){
    $bnkid = $_POST['bkid'];
    $bnkname =$_POST['bkname'];

    $sql= "SELECT *FROM banks WHERE bank_id = '$bnkid' AND bnk_name = '$bnkname'";
    ...
    if(mysqli_fetch_assoc( $result)>1){
        ...
        $sql = "INSERT INTO banks(bank_id, bnk_name) VALUES ('$bnkid','$bnkname')";

        $result = mysqli_query($conn, $sql);

        if ($result){
            echo  "<script> alert( ' THE BANK REGISTERED SUCCESSFULLY') </script>";
     }else{ 
    ...
    <form action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>" method="post">
      ...
      <div class="form-row">
        <div class="col">
          ...
          <label for="bank id"><b>Set Bank Id:</b></label>
          <input type="text" placeholder="Enter the bank id" name="bkid" required>
        </div>
      </div>
      ...
      <div class="form-row">
        <div class="col">
          ...
          <label for="bank name"><b>Set Bank Name:</b></label>
          <input type="text" placeholder="Enter the bank name" name="bkname" required>
        </div>
      </div>
