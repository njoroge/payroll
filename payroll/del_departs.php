...
if(isset($_POST['submit'])){
    ...
    $dptname =$_POST['depart'];
    $dpstate = $_POST['state'];

    $sql= "SELECT *FROM departments WHERE depart_name ='$dptname' AND depart_status='$dpstate'";

    $result = mysqli_query($conn, $sql);
    if(mysqli_fetch_assoc( $result)>1){
        echo "<script> alert('The state already set') </script>";
    }
    elseif ($dpstate==='ACTIVE') {
        $sql = "UPDATE departments SET depart_status='ACTIVE'";
        $result = mysqli_query($sql, $conn);
        if($result){
            echo "<script> alert('The  successfully changed') </script>";
        }
        else{
            echo "<script> alert('Failed!') </script>"; 
        }
    }
    elseif ($dpstate==='INACTIVE') {
        $sql = "UPDATE departments SET depart_status='INACTIVE'";
        $result = mysqli_query($sql, $conn);
        if($result){
            echo "<script> alert('The  successfully changed') </script>";
        }
...
<body>
  ...
  <div>
    <form action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>" method="post">
      ...
      <div class="form-row">
        <div class="col">
        <label for="set status"><b>Select Action:</b></label>
      <select name="state">
